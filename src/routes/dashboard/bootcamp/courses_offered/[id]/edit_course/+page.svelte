<script lang="ts">
  ...
  let selectedName = employeeNames.find(
    (employeeNames) => employeeNames.Employee_ID === course.Employee_ID,
  ).names;
  ...
  function getEmployeeID(selectedName: string): string {
    const employee = employeeNames.find(
      (employeeNames) => employeeNames.names === selectedName,
    );
  ...
  $: courseSched2Formatted = new Date(
    `1970-01-01T${courseSched2}:00`,
  ).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
  $: courseSched3Formatted = new Date(
    `1970-01-01T${courseSched3}:00`,
  ).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
  $: courseSchedule =
    courseSched1 + " " + courseSched2Formatted + " - " + courseSched3Formatted;
  $: courseDuration =
    courseDur + (courseDur === 1 ? " month" : " months") + " (6 hours/week)";
</script>
  ...
  <form method="POST" class="mt-6">
    ...
    <label for="courseDuration">
      ...
      <input
        ...
        name="courseDuration"
        id="courseDuration"
        bind:value={courseDuration}
      />
      <label for="courseSlots"> Course Capacity: </label>
      <br />
      <input
        class="text-sm border-2 rounded-full border-gray-400 p-1 pl-2 pr-2"
        type="number"
      ...
      <div style="display: flex; justify-content: space-between; width: 75%">
        ...
        <a
          href="/dashboard/bootcamp/courses_offered/{course.Course_ID}/delete_course"
          class="mr-20"
        >
          <button type="button">
