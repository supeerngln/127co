...
function getEmployee(formData) {
  return {
    ...
    Employee_ReportsTo:
      formData.get("reports-to").length > 0
...
async function getPDS(formData) {
  ...
  return {
    ...
    Employee_HighestLevelOfEducation:
      formData.get("highest-level-of-education") ?? "",
    ...
    Emergency_ContactRelationship: formData.get(
...
function addEmployee(employee, employeeId) {
  ...
  const values = [
    ...
    employee.Employee_DateOfHire,
    employee.Employee_ReportsTo,
  ];
...
function addPDS(pds, employeeId) {
  ...
  const fields = [
    ...
    "Employee_HighestLevelOfEducation",
    "Employee_School",
    "Employee_YearGraduated",
    "Employee_PhilHealthID",
    "Employee_SSSID",
    "Employee_PagIBIGID",
  ...
  const values = [
    employeeId,
    pds.Employee_Email,
    pds.Employee_Address,
    pds.Employee_ContactNumber,
    pds.Employee_DateOfBirth,
    pds.Employee_PlaceOfBirth,
    pds.Employee_Sex,
    pds.Employee_CivilStatus,
    pds.Employee_Citizenship,
    pds.Employee_HighestLevelOfEducation,
    pds.Employee_School,
    pds.Employee_YearGraduated,
    pds.Employee_PhilHealthID,
    pds.Employee_SSSID,
    pds.Employee_PagIBIGID,
    pds.Emergency_ContactName,
    pds.Emergency_ContactRelationship,
    pds.Emergency_ContactNumber,
  ];
...
async function validateData(employee, pds) {
  ...
  if (
    employee.Employee_MiddleName &&
    !hasNoNumbers(employee.Employee_MiddleName)
  ) {
    result.pass = false;
  ...
  if (
    ...
    !(await employeeIDExists(employee.Employee_ReportsTo))
  ) {
    result.pass = false;
  ...
  if (
    pds.Employee_ContactNumber &&
    !isValidPhoneNumber(pds.Employee_ContactNumber)
  ) {
    result.pass = false;
  ...
  if (
    pds.Emergency_ContactNumber &&
    !isValidPhoneNumber(pds.Emergency_ContactNumber)
  ) {
    result.pass = false;
  ...
  if (
    pds.Employee_CivilStatus &&
    !civilStatusOptions.includes(pds.Employee_CivilStatus)
  ) {
    result.pass = false;
  ...
  if (
    pds.Employee_HighestLevelOfEducation &&
    !highestLevelOfEducationOptions.includes(
      pds.Employee_HighestLevelOfEducation,
    )
  ) {
    result.pass = false;
