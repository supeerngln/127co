<script lang="ts">
  ...
  $: if (query || selected_type || selected_status) {
    filtered_proj = data.projects.filter((project: any) =>
      ...
      (project.Project_Type.toLowerCase().includes(selected_type.toLowerCase()) || selected_type === "") &&
      (project.Project_Status.toLowerCase().includes(selected_status.toLowerCase()) || selected_status === "")
    );
    show_add = false;
  } else {
...
<div class="container">
  <Breadcrumb items={[{ href: "/dashboard/projects", text: "Projects" }]} />
  <div class="flex items-center justify-center">
    <div class="flex items-center mr-2">
      <input
        ...
        bind:value={query}
      />
      <Menu name="Select Status" items={["In Development", "For Deployment", "For Monitoring", "For Presentation", "Done", "Terminated"]} bind:selected_item={selected_status} />
      <Menu name="Select Type" items={["Mobile Application", "Web Application", "Standalone Systems"]} bind:selected_item={selected_type} />
    </div>
  </div>
  <div class="project grid grid-cols-3 gap-5">
    {#if show_add}
      <a
        href="/dashboard/projects/create" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
      >
