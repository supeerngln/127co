<script lang="ts">
  ...
  <div>
    <Breadcrumb
      items={[
        { href: "/dashboard/software", text: "Software" },
        {
          ...
          text: data.software.Software_Name + " " + data.software.Software_Version,
        },
        {
    ...
    />
    <form class="grid grid-cols-3 gap-4" method="POST" use:enhance={ ({formData, cancel}) => {
      ...
      <div class="flex items-center">
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2" for="name">Version:</label>
        <input
          class="text-sm border-2 rounded-full border-gray-400 text-gray-700 p-1 pl-2 pr-2"
          type="text"
          id="version"
      ...
      <div class="flex items-center col-span-3">
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2" for="os">Operating System:</label>
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2">
          <input type="radio" name="platform" value="Web-based" on:change={solo} >
          Web-based
        </label>
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2">
          <input type="radio" name="platform" value="Cross-platform" on:change={solo}>
          Cross-platform
        </label>
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2 flex items-center">
          <input type="radio" name="platform" value="Desktop" on:change={multiple} required>
            Desktop
          {#if show}
      ...
      <div class="flex items-center">
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2" for="name">Publisher:</label>
        <input
          class="text-sm border-2 rounded-full border-gray-400 text-gray-700 p-1 pl-2 pr-2 w-full"
          type="text"
          id="publisher"
      ...
      <div class="flex items-center">
        <label class="text-sm text-gray-900 p-1 pl-2 pr-2" for="team">License:</label>
        <select
          class="text-sm border-2 rounded-full border-gray-400 text-gray-700 p-1 pl-2 pr-2"
          id="license"
          name="license"
      ...
      <div class="flex justify-end col-span-2 p-2 space-x-2">
        ...
        <a
          href="/dashboard/projects/{data.software.Software_Name}_{data.software.Software_Version}"
          class={"flex text-sm my-0.5 mx-4 rounded-md items-center space-x-3 py-2 px-4 transition duration-200 hover:bg-phover hover:text-secondary focus:text-accent focus:bg-afocus"}
        >
        ...
        <a
          href="/dashboard/projects/{data.software.Software_Name}_{data.software.Software_Version}/delete"
          class={"flex text-sm my-0.5 mx-4 rounded-md items-center space-x-3 py-2 px-4 transition duration-200 hover:bg-phover hover:text-secondary focus:text-accent focus:bg-afocus"}
        >
